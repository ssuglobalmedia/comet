...
export const toLogDao = (log: Log): LogDao => ({
  ...
  t: { S: log.target },
  a: { S: log.action },
  ...(log.description && { d: { S: log.description } })
})

export const queryLogs = async (year: number = undefined, month: number = undefined, day: number = undefined): Promise<Array<Log>> => {
  let composedRes: Array<Log> = [];
  const date = year ? month ? day ? `${String(year).padStart(4, '0')}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}` : `${String(year).padStart(4, '0')}-${String(month + 1).padStart(2, '0')}` : `${String(year).padStart(4, '0')}` : '';
