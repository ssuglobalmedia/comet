...
export const fromUserDao = (dao: UserDao): User => ({
	...
	...dao.lastSemester?.N && {lastSemester: (dao.lastSemester.N < 0 ? `${dao.lastSemester.N * -1}-1` : `${dao.lastSemester.N}-2`)},
...
export const updateUser = async function (
	...
	if(info.lastSemester) {
		const sem = parseInt(info.lastSemester.split('-')[0]) * (info.lastSemester.split('-')[1] === "1" ? -1 : 1)
