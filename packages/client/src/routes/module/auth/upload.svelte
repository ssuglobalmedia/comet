...
<ProgressIndicator preventChangeOnClick={currentIndex > 2} spaceEqually class="my-4" bind:currentIndex>
  ...
  <ProgressStep complete={currentIndex > 3 && response && response.success} disabled={currentIndex < 3} label="업로드" />
</ProgressIndicator>
{#if currentIndex === 0}
  <StepTile
    title="파일 업로드"
...
{:else if currentIndex === 2}
  ...
  >
    ...
    />
    <Button slot="navigation" on:click={() => {if(currentIndex < 3) currentIndex = 3}} icon={Upload16}>등록하기</Button>
  </StepTile>
{:else}
  <Tile>
    <h1>등록</h1>
