<script lang="ts">
  ...
  $: groupColValues = originalData
    ? [
        ...originalData.reduce<Set<string>>((set, v) => {
  ...
  function formValidation(): boolean {
    ...
    if (groupCertificatedValue.length) {
      ...
      desiredSize += 1;
      if (groupComboBoxItems[groupCertificatedIndex].id === groupColValues.length)
        defaultSet = true;
    }
    ...
    if (formValidation()) {
      conversion = {
        ...
        userGroupConv: {
          everyone: {
            defaults:
              groupEveryoneIndex > -1 &&
              groupComboBoxItems[groupEveryoneIndex].id === groupColValues.length,
            keyword: groupEveryoneValue ? groupEveryoneValue : undefined,
          },
          certificated: {
            defaults:
              groupCertificatedIndex > -1 &&
              groupComboBoxItems[groupCertificatedIndex].id === groupColValues.length,
            keyword: groupCertificatedValue ? groupCertificatedValue : undefined,
          },
          executive: {
            defaults:
              groupExecutiveIndex > -1 &&
              groupComboBoxItems[groupExecutiveIndex].id === groupColValues.length,
            keyword: groupExecutiveValue ? groupExecutiveValue : undefined,
          },
          admin: {
            defaults:
              groupAdminIndex > -1 &&
              groupComboBoxItems[groupAdminIndex].id === groupColValues.length,
        ...
        ...(includeLastSemester && {
          lastSemester:
            lastSemesterCol >= dropdownItems.length
              ? ':latest'
              : dropdownItems[lastSemesterCol].text,
        }),
...
<p class="bx--data-table-header__description">
  열람한 데이터를 형식에 맞게 변환해 주세요.<br />COMET 사용자 데이터의 행 제목과 업로드한 데이터의
  행 제목을 일치시켜 주고, 해당 데이터셋에 존재하는 정보는 체크해 주세요.
</p>
<div style="margin-top: 1.5rem">
  <FormGroup legendText="행 선택">
    <Dropdown
      ...
      invalid={userIdCol === userNameCol ||
        userIdCol === userGroupCol ||
        (includePhone && userIdCol === phoneCol) ||
        (includeLastSemester && userIdCol === lastSemesterCol)}
    ...
    <Dropdown
      ...
      invalid={userNameCol === userIdCol ||
        userNameCol === userGroupCol ||
        (includePhone && userNameCol === phoneCol) ||
        (includeLastSemester && userNameCol === lastSemesterCol)}
      invalidText="속성에 대해 선택한 행이 다른 속성과 겹칩니다." />
    <Dropdown
      titleText="구분"
      bind:selectedId={userGroupCol}
      items={dropdownItems}
      invalid={userGroupCol === userIdCol ||
        userGroupCol === userNameCol ||
        (includePhone && userGroupCol === phoneCol) ||
        (includeLastSemester && userGroupCol === lastSemesterCol)}
      invalidText="속성에 대해 선택한 행이 다른 속성과 겹칩니다."
    ...
    <div style="display: block">
      <div style="display: flex; align-items: center;">
        ...
        <Dropdown
          ...
          items={dropdownItems}
          invalid={includePhone &&
            (phoneCol === userIdCol ||
              phoneCol === userNameCol ||
              phoneCol === userGroupCol ||
              (includeLastSemester && phoneCol === lastSemesterCol))}
          invalidText="속성에 대해 선택한 행이 다른 속성과 겹칩니다." />
      </div>
      <div style="display: flex; align-items: center;">
        <Checkbox
          style="flex: 0 1 auto; margin-right: 0.875rem;"
          labelText="마지막 재학 학기 포함"
          hideLabel
        ...
        <Dropdown
          ...
          invalid={includeLastSemester &&
            (lastSemesterCol === userIdCol ||
              lastSemesterCol === userNameCol ||
              lastSemesterCol === userGroupCol ||
              (includePhone && lastSemesterCol === phoneCol))}
          invalidText="속성에 대해 선택한 행이 다른 속성과 겹칩니다." />
      </div>
      <p class="bx--label">
        해당 속성의 값은 년도-학기를 숫자로 표현한 값이여야 합니다. 예) 2022-1
      </p>
    </div>
  </FormGroup>
  <FormGroup legendText="변환 선택">
    ...
    <ComboBox
      ...
      invalid={groupEveryoneValue.length > 0 &&
        (groupEveryoneValue === groupCertificatedValue ||
          groupEveryoneValue === groupExecutiveValue ||
          groupEveryoneValue === groupAdminValue)}
    ...
    <ComboBox
      ...
      invalid={groupCertificatedValue.length > 0 &&
        (groupCertificatedValue === groupEveryoneValue ||
          groupCertificatedValue === groupExecutiveValue ||
          groupCertificatedValue === groupAdminValue)}
      invalidText="다른 권한에 동일한 값을 선택할 수 없습니다."
      {shouldFilterItem} />
    <ComboBox
      titleText="집행부원"
      ...
      invalid={groupExecutiveValue.length > 0 &&
        (groupExecutiveValue === groupEveryoneValue ||
          groupExecutiveValue === groupCertificatedValue ||
          groupExecutiveValue === groupAdminValue)}
      invalidText="다른 권한에 동일한 값을 선택할 수 없습니다."
      {shouldFilterItem} />
    <ComboBox
      titleText="관리자"
      ...
      invalid={groupAdminValue.length > 0 &&
        (groupAdminValue === groupEveryoneValue ||
          groupAdminIndex === groupCertificatedValue ||
          groupAdminValue === groupExecutiveValue)}
      invalidText="다른 권한에 동일한 값을 선택할 수 없습니다."
      {shouldFilterItem} />
  </FormGroup>
