<script lang="ts">
  import {
    Button,
    ComposedModal, DatePicker, DatePickerInput, FormGroup, InlineLoading,
    ModalBody, ModalFooter, ModalHeader, MultiSelect, Select, SelectItem,
    TextInput
  ...
  function queryUser() {
    ...
    fetchWithAuth(variables.baseUrl + `/api/module/auth/user/query?starts=${userId}`).then((res) => res.json()).then((res: CometResponse) => {
...
<ComposedModal bind:open on:submit={doRent} {...$$restProps}>
  ...
  <ModalBody hasForm>
    <FormGroup legendText="사용자">
      ...
      <Button kind="secondary" disabled={userReqStatus !== "pending"} on:click={queryUser}>
        ...
        {:else if userReqStatus === "finished"}
          <InlineLoading status="finished" description="사용자 {user.userName}님을 찾았습니다. / {groupDisplayName[user.userGroup]}" />
    ...
    <TextInput labelText="비고(보증 물품)" placeholder="학생증, 민증 등 특이사항 입력..." bind:value={additionalInfo} />
  </ModalBody>
  ...
  <ModalFooter
      primaryButtonDisabled={!user || selectedGoodies.length <= 0 ||
    reqStatus !== 'pending'}
