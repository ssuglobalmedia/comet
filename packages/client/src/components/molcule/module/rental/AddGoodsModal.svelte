...
<ComposedModal bind:open on:submit={doAdd} {...$$restProps}>
  ...
  <ModalBody hasForm>
    <TextInput labelText="물품 ID" placeholder="물품 ID를 입력하세요..." helperText="알파벳(a-Z)와 언더스코어(_)만 허용됩니다."
               bind:value={goodsId}/>
  ...
  <ModalFooter
      primaryButtonDisabled={!goodsId || !name || !category || !permission ||
    reqStatus !== 'pending'}
